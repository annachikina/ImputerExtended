...
/**
 * Params for [[ImputerNum]] and [[ImputerNumModel]].
 */
private[feature] trait ImputerNumParams extends Params with HasInputCol with HasInputCols
  ...
  final val strategy: Param[String] = new Param(this, "strategy", s"strategy for imputation. " +
    s"If ${ImputerNum.mean}, then replace missing values using the mean value of the feature. " +
    s"If ${ImputerNum.median}, then replace missing values using the median value of the feature. " +
    s"If ${ImputerNum.const}, then replace missing values using fill value.",
    ParamValidators.inArray[String](Array(ImputerNum.mean, ImputerNum.median, ImputerNum.const)))
  ...
  /**
   * The placeholder for the fill value. All occurrences of missingValue will be imputed with fillValue if strategy="const".
   * Note that null values are always treated as missing.
   * Default: Double.NaN
   *
   * @group param
   */
  final val fillValue: DoubleParam = new DoubleParam(this, "fillValue",
    "The placeholder for the fill value. All occurrences of missingValue will be imputed with fillValue if strategy=\"const\".")

  /** @group getParam */
  def getFillValue: Double = $(fillValue)

  setDefault(strategy -> ImputerNum.mean, missingValue -> Double.NaN, fillValue -> 0.0)
 ...
 * numeric type. Currently ImputerNum does not support categorical features
 ...
 */
@Since("2.2.0")
class ImputerNum @Since("2.2.0")(@Since("2.2.0") override val uid: String)
  extends Estimator[ImputerNumModel] with ImputerNumParams with DefaultParamsWritable {
  ...
  /**
   * Imputation strategy. Available options are ["mean", "median", "const"].
   *
   * @group setParam
   */
  @Since("2.2.0")
  ...
  override def fit(dataset: Dataset[_]): ImputerNumModel = {
    ...
    val results = $(strategy) match {
      ...
      case ImputerNum.const =>
        Array.fill(inputColumns.length)($(fillValue))
    }
    ...
    copyValues(new ImputerNumModel(uid, surrogateDF).setParent(this))
  }
  ...
  override def copy(extra: ParamMap): ImputerNum = defaultCopy(extra)
}

@Since("2.2.0")
object ImputerNum extends DefaultParamsReadable[ImputerNum] {

  /** strategy names that ImputerNum currently supports. */
  private[feature] val mean = "mean"
  private[feature] val median = "median"
  private[feature] val const = "const"

  @Since("2.2.0")
  override def load(path: String): ImputerNum = super.load(path)
}

/**
 * Model fitted by [[ImputerNum]].
 *
 ...
 */
@Since("2.2.0")
class ImputerNumModel private[ml](
                                   @Since("2.2.0") override val uid: String,
                                   @Since("2.2.0") val surrogateDF: DataFrame)
  extends Model[ImputerNumModel] with ImputerNumParams with MLWritable {

  import ImputerNumModel._
  ...
  override def copy(extra: ParamMap): ImputerNumModel = {
    val copied = new ImputerNumModel(uid, surrogateDF)
    copyValues(copied, extra).setParent(parent)
  }

  @Since("2.2.0")
  override def write: MLWriter = new ImputerNumModelWriter(this)

  @Since("3.0.0")
  override def toString: String = {
    s"ImputerNumModel: uid=$uid, strategy=${$(strategy)}, missingValue=${$(missingValue)}, fillValue=${$(fillValue)}" +
...
@Since("2.2.0")
object ImputerNumModel extends MLReadable[ImputerNumModel] {

  private[ImputerNumModel] class ImputerNumModelWriter(instance: ImputerNumModel) extends MLWriter {
  ...
  private class ImputerNumReader extends MLReader[ImputerNumModel] {

    private val className = classOf[ImputerNumModel].getName

    override def load(path: String): ImputerNumModel = {
      val metadata = DefaultParamsReader.loadMetadata(path, sc, className)
      val dataPath = new Path(path, "data").toString
      val surrogateDF = sqlContext.read.parquet(dataPath)
      val model = new ImputerNumModel(metadata.uid, surrogateDF)
  ...
  @Since("2.2.0")
  override def read: MLReader[ImputerNumModel] = new ImputerNumReader

  @Since("2.2.0")
  override def load(path: String): ImputerNumModel = super.load(path)
}